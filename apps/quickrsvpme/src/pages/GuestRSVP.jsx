  ...
  return (
    ...
    >
      <style>{`
        ...
        .animate-scene { animation: fadeInScale 0.8s cubic-bezier(0.16, 1, 0.3, 1) forwards; }
      `}</style>
      ...
      <main className="relative z-20 flex flex-col justify-center h-[calc(100dvh-180px)] px-6 max-w-md mx-auto text-center">
        {activeScene === 0 && (
          <div key="scene-0" className="animate-scene space-y-5">
            <div className="text-sm font-bold text-[#D4AF37] tracking-widest">﷽</div>
            <h2 className="text-2xl font-bold font-header leading-relaxed text-[#FBF9F5]">"بسم الله الرحمن الرحيم"</h2>
            <p className="text-xs leading-relaxed text-neutral-300 font-body px-2 opacity-90">{guestInfo.poetryVerse}</p>
          </div>
        )}

        {activeScene === 1 && (
          <div key="scene-1" className="animate-scene space-y-4">
            <span className="text-xs font-bold uppercase tracking-widest text-[#D4AF37]">يتشرفان بدعوتكم لحفل زفافهما</span>
            <div className="space-y-1">
              <h1 className="text-4xl font-bold font-header text-white leading-tight">{guestInfo.groomName}</h1>
              <span className="text-xl font-serif text-[#D4AF37] block">&amp;</span>
              <h1 className="text-4xl font-bold font-header text-white leading-tight">{guestInfo.brideName}</h1>
            </div>
            <p className="text-xs text-neutral-300 pt-2">أهلاً بك <span className="font-bold text-[#D4AF37]">{guestInfo.name}</span> في ليلتنا المباركة</p>
          </div>
        )}
        ...
        {(activeScene === 2 || activeScene >= 3) && (
          <div key="scene-2" className="animate-scene space-y-5">
            <div className="space-y-2">
              <span className="text-xs font-bold text-[#D4AF37] tracking-widest uppercase block">🗓️ {guestInfo.eventDate}</span>
              <h1 className="text-3xl font-bold font-header text-white leading-tight">{guestInfo.eventTitle}</h1>
        ...
        <div className="absolute bottom-6 left-0 right-0 flex justify-center gap-2">
          {[0, 1, 2].map((idx) => (
            <button
              ...
              className={`h-1.5 rounded-full transition-all duration-300 ${activeScene === idx ? 'w-6 bg-[#D4AF37]' : 'w-1.5 bg-white/30'}`}
            />
          ))}
        </div>
      </main>

      {/* LAYER 2: BOTTOM DRAWER & BUTTON */}
      {!isSubmitted && (
        <div className="fixed bottom-6 left-0 right-0 z-30 flex justify-center px-4">
          <button
            onClick={() => setIsDrawerOpen(true)}
            className="w-full max-w-xs py-4 rounded-2xl text-[#071913] font-bold text-sm shadow-2xl hover:brightness-110 transition flex items-center justify-center gap-2 border border-amber-300/40"
            style={{ background: `linear-gradient(135deg, ${eventConfig.primary_color} 0%, #B89628 100%)` }}
          >
            <span>✨</span>
      ...
      {isDrawerOpen && (
        <div className="fixed inset-0 z-40 bg-black/60 backdrop-blur-sm flex items-end justify-center">
          ...
          <div
            className="relative z-50 w-full max-w-md p-6 rounded-t-3xl backdrop-blur-2xl border-t text-white space-y-5 shadow-2xl"
            style={{ backgroundColor: `${eventConfig.secondary_color}E6`, borderColor: `${eventConfig.primary_color}40` }}
          >
            ...
            <form onSubmit={handleSubmitRSVP} className="space-y-4 text-xs font-body">
              <div className="space-y-1.5">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <button
                    ...
                    className={`py-3.5 px-4 rounded-xl border text-xs font-bold transition ${attendance === 'attending' ? 'bg-[#D4AF37] text-[#071913] border-[#D4AF37]' : 'bg-black/40 text-neutral-400 border-white/10'}`}
                  >
                    ✓ يشرفني الحضور
                  </button>
                  <button
                    ...
                    className={`py-3.5 px-4 rounded-xl border text-xs font-bold transition ${attendance === 'declined' ? 'bg-rose-700 text-white border-rose-600' : 'bg-black/40 text-neutral-400 border-white/10'}`}
                  >
                    ✕ أعتذر عن الحضور
                  </button>
                </div>
              ...
              {attendance === 'attending' && (
                <div className="space-y-1.5 p-3.5 rounded-2xl bg-black/40 border border-white/10 flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-3 bg-black/60 px-3 py-1.5 rounded-xl border border-white/10">
                    <button type="button" onClick={() => setGuestCount(Math.max(1, guestCount - 1))} className="w-7 h-7 rounded-lg bg-white/10 text-white font-bold text-sm flex items-center justify-center">-</button>
                    <span className="font-bold text-[#D4AF37] text-base min-w-[20px] text-center">{guestCount}</span>
                    <button type="button" onClick={() => setGuestCount(Math.min(eventConfig.allowed_plus_ones, guestCount + 1))} className="w-7 h-7 rounded-lg bg-white/10 text-white font-bold text-sm flex items-center justify-center">+</button>
                  </div>
                </div>
              )}
              ...
              <div className="space-y-1.5">
                ...
                <textarea
                  ...
                  placeholder={attendance === 'attending' ? 'اكتب أمنياتك ودعواتك الطيبة للعروسين...' : 'اكتب كلمة طيبة أو اعتذارك للعائلة...'}
                  rows="3"
              ...
              <div className="pt-2 flex gap-3">
                <button type="button" onClick={() => setIsDrawerOpen(false)} className="w-1/3 py-3 rounded-xl bg-black/40 border border-white/10 font-bold text-neutral-400">إلغاء</button>
                <button
                  type="submit"
                  disabled={isSubmitting}
                  className="w-2/3 py-3.5 rounded-xl text-[#071913] font-bold text-xs shadow-xl disabled:opacity-50"
                  style={{ background: `linear-gradient(135deg, ${eventConfig.primary_color} 0%, #B89628 100%)` }}
                >
      ...
      {/* LAYER 3: QR CONFIRMATION MODAL */}
      {isSubmitted && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-4">
          <div className="w-full max-w-sm p-6 rounded-3xl border text-center space-y-5 shadow-2xl" style={{ backgroundColor: eventConfig.secondary_color, borderColor: `${eventConfig.primary_color}60` }}>
            <div className="w-12 h-12 rounded-full flex items-center justify-center text-2xl mx-auto shadow-md" style={{ backgroundColor: `${eventConfig.primary_color}20`, borderColor: eventConfig.primary_color, color: eventConfig.primary_color }}>
              ✓
            </div>
            <div className="space-y-1">
              <span className="text-[10px] font-bold uppercase tracking-widest text-[#D4AF37]">تأكيد الرد بنجاح</span>
            ...
            {attendance === 'attending' && (
              <div className="p-4 rounded-2xl bg-black/50 border border-[#D4AF37]/30 space-y-3">
                <div className="text-[11px] font-bold text-[#D4AF37] uppercase tracking-wider">بطاقة الدخول الرقمية (VIP Digital Pass)</div>
                ...
                <div className="text-xs font-bold text-white pt-1">{guestInfo.name} (+{guestCount - 1})</div>
                <div className="text-[10px] font-mono text-neutral-400">Token: {guestInfo.token}</div>
              </div>
            )}

            <button onClick={() => setIsSubmitted(false)} className="w-full py-3 rounded-xl bg-black/40 border border-white/10 text-xs font-bold text-neutral-300">
              إغلاق الشاشة
            </button>
          </div>
        </div>
